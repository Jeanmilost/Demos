 ...
 * ==> QR_MD3Animation -------------------------------------------------------*
 ******************************************************************************
 * Description : Helper class for MD3 model                                   *
...
//------------------------------------------------------------------------------
// QR_MD3Animation::IAnimation - c++ cross-platform
//------------------------------------------------------------------------------
QR_MD3Animation::IAnimation::IAnimation() :
    m_FrameIndex(M_MD3_Invalid_Index),
    m_InterpolationFrameIndex(M_MD3_Invalid_Index),
    m_InterpolationFactor(0.0f)
{}
//------------------------------------------------------------------------------
QR_MD3Animation::IAnimation::IAnimation(QR_SizeT frameIndex, QR_SizeT interpolationFrameIndex,
...
{}
//------------------------------------------------------------------------------
// QR_MD3Animation::IInfo - c++ cross-platform
//------------------------------------------------------------------------------
QR_MD3Animation::IInfo::IInfo() : m_Start(0), m_End(0)
{}
//------------------------------------------------------------------------------
QR_MD3Animation::IInfo::IInfo(QR_SizeT start, QR_SizeT end) : m_Start(start), m_End(end)
{}
//------------------------------------------------------------------------------
QR_MD3Animation::IInfo::~IInfo()
{}
//------------------------------------------------------------------------------
// QR_MD3Animation - c++ cross-platform
//------------------------------------------------------------------------------
QR_MD3Animation::QR_MD3Animation(QR_MD3* pModel, bool useCache) :
    m_pModel(pModel),
    m_pInfo(NULL),
    m_FrameIndex(0),
    m_InterpolationFactor(0.0f)
...
//------------------------------------------------------------------------------
QR_MD3Animation::~QR_MD3Animation()
{
...
//------------------------------------------------------------------------------
void QR_MD3Animation::SetUseCache(bool value)
{
    // get cache count
    const QR_SizeT cacheCount = m_Cache.size();
...
//------------------------------------------------------------------------------
bool QR_MD3Animation::Add(const std::string& name, QR_SizeT start, QR_SizeT end)
{
    // no name?
    if (name.empty())
        return false;
...
//------------------------------------------------------------------------------
void QR_MD3Animation::Delete(const std::string& name)
{
    // no name?
    if (name.empty())
        return;
...
//------------------------------------------------------------------------------
bool QR_MD3Animation::Set(const std::string& name)
{
    // no name?
    if (name.empty())
        return false;
...
//------------------------------------------------------------------------------
std::string QR_MD3Animation::GetRunningAnimationName() const
{
    return m_CurAnimName;
}
//------------------------------------------------------------------------------
QR_SizeT QR_MD3Animation::GetRunningAnimationStartIndex() const
{
...
//------------------------------------------------------------------------------
QR_SizeT QR_MD3Animation::GetRunningAnimationEndIndex() const
{
    // no running animation?
    if (!m_pInfo)
        return 0;
...
//------------------------------------------------------------------------------
QR_MD3Animation::IAnimation QR_MD3Animation::GetAnimation(const QR_Double& elapsedTime,
                                                                QR_SizeT   fps)
{
    // no animation info?
    if (!m_pInfo)
...
//------------------------------------------------------------------------------
bool QR_MD3Animation::GetMesh(const QR_Double& elapsedTime,
                                    QR_SizeT   fps,
                                    QR_Mesh&   mesh)
{
    // no model?
    ...
    // found it?
    if (animation.m_FrameIndex == M_MD3_Invalid_Index)
        return false;
    ...
    if (!m_Cache.size())
    {
        ...
        try
        {
            ...
            success = (QR_MD3::Interpolate(animation.m_InterpolationFactor,
                                           frameMesh,
                                           interpolationMesh,
                                           mesh) == QR_MD3Common::IE_C_Success);
        }
        catch (...)
        {}
    ...
    // process interpolation and send resulting mesh to output
    return (QR_MD3::Interpolate(animation.m_InterpolationFactor,
                                m_Cache[animation.m_FrameIndex],
                                m_Cache[animation.m_InterpolationFrameIndex],
                                mesh) == QR_MD3Common::IE_C_Success);
}
//------------------------------------------------------------------------------
bool QR_MD3Animation::GetFrames(QR_SizeT start, QR_SizeT end, IFrames& frames) const
{
    // no model?
    if (!m_pModel)
        return false;
...
//------------------------------------------------------------------------------
bool QR_MD3Animation::GetFrames(IFrameDictionary& frameDict) const
{
